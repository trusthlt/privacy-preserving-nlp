...
\subtitle{Lecture 10 : Real-World DP Applications and the Epsilon Registry}
\date{July 9, 2026}
...
\begin{frame}{Google Gboard next-word prediction}

Federated learning and DP to build next-word prediction models for Gboard, a virtual keyboard for android. 

\begin{itemize} \setlength\itemsep{2mm}
%
\item Each model uses $(\varepsilon, \delta)$ DP with:
%
\begin{itemize} \setlength\itemsep{1mm}
%
\item $\varepsilon$ from 0.69 to 10.61
%
\item $\delta = 10^{-5}$
%
\end{itemize}
%
\item Models trained with TensorFlow Federated\footnote{\url{https://www.tensorflow.org/federated}} and TensorFlow Privacy\footnote{\url{https://www.tensorflow.org/responsible_ai/privacy}}
%
\end{itemize}

\begin{tikzpicture}[overlay, remember picture]
\node at (current page.north east)[ref] {
\fullcite{xu2023federatedlearninggboardlanguage} \par};
\end{tikzpicture}
...
\begin{frame}{Gboard out-of-vocabulary word discovery}

Distributed DP for discovering new words (out-of-vocabulary) typed by users in Gboard, and enhance the preformance of the keyboard. \vskip2mm 

\begin{itemize} \setlength\itemsep{3mm}
%
\item Local models using $\varepsilon$ DP with $\varepsilon = 10$  
%
\item Central guarantee $(\varepsilon, \delta)$ DP with $\varepsilon = 0.32$ and $\delta = 10^{-10}$ 
%
\end{itemize}

Full details are reported {\it \href{https://research.google/blog/improving-gboard-language-models-via-private-federated-analytics/}{here}}.

\begin{tikzpicture}[overlay, remember picture]
\node at (current page.north east)[ref] {
\fullcite{sun2024privatefederateddiscoveryoutofvocabulary} \par};
\end{tikzpicture}
